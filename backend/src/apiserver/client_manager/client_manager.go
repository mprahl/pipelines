...
func (c *ClientManager) init(useKubernetesStorage bool) {
	...
	if useKubernetesStorage {
		...
		if err != nil {
			panic(fmt.Sprintf("Failed to generate a cache to the cluster: %v", err))
		}

		go func() {
			// TODO: Add real context
			err := k8sAPICache.Start(context.TODO())
			if err != nil {
		...
		if err != nil {
			panic(fmt.Sprintf("Failed to initialize the client: %v", err))
		}

		c.controllerClient = controllerClient

		c.pipelineStore = storage.NewKubernetesPipelineStore(controllerClient)
...
func NewClientManager(useKubernetesStorage bool) ClientManager {
	clientManager := ClientManager{}
	clientManager.init(useKubernetesStorage)
