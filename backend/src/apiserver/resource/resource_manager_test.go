...
// setupTestSAToken writes content to a temp file and overrides ServiceAccountTokenPath
// for the duration of the test.
func setupTestSAToken(t *testing.T, content string) {
	t.Helper()
...
func TestCreateRun_WithMLflowPlugin(t *testing.T) {
	// Set up a fake MLflow server that handles experiment lookup, run creation, and tagging.
	var (
	...
	mlflowServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/2.0/mlflow/runs/set-tag":
			...
			tagCalls = append(tagCalls, string(body))
			w.WriteHeader(http.StatusOK)
			_, _ = w.Write([]byte(`{}`))
		default:
			http.NotFound(w, r)
	...
	assert.NotEmpty(t, tagCalls, "MLflow tag calls should have been made")
	...
	output, err := apiservermlflow.GetRunPluginOutput(storedRun, apiservermlflow.PluginName)
	require.NoError(t, err)
	...
	// Verify the workflow got KFP_MLFLOW_CONFIG env var injected.
	// The runtime manifest stored on the run is serialized after env injection.
	execSpec, err := store.ExecClientFake.Execution("ns1").Get(context.Background(), runDetail.K8SName, v1.GetOptions{})
	require.NoError(t, err)
	require.NotNil(t, execSpec)
	runtimeManifest := execSpec.ToStringForStore()
	assert.Contains(t, runtimeManifest, commonmlflow.EnvMLflowConfig,
		"Workflow should contain KFP_MLFLOW_CONFIG env var")
}
...
// set, reporting a terminal workflow triggers
// applyMLflowOnRunEnd, which updates the plugin output state.
func TestReportWorkflow_WithMLflowOnRunEnd(t *testing.T) {
	// Ensure no global MLflow config — OnRunEnd should still be triggered and
	// should set PLUGIN_FAILED because config is unavailable.
	...
	// Pre-populate PluginsOutputString to simulate a prior OnRunStart success.
	...
	// After terminal report, applyMLflowOnRunEnd should have fired.
