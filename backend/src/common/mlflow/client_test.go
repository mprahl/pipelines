...
func TestApplyHeaders_BearerAuth(t *testing.T) {
...
func TestApplyHeaders_KubernetesAuth_UsesBearer(t *testing.T) {
	c, _ := NewClient(Config{
		Endpoint:    "http://mlflow.example.com",
...
func TestDoWithRetry_RetriesOn5xx(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"experiment_id":"ok-after-retry"}`))
	}))
	defer server.Close()

	c := newTestClientWithFastRetry(t, server.URL)
	id, err := c.CreateExperiment(context.Background(), "retry-exp", nil)
	require.NoError(t, err)
	assert.Equal(t, "ok-after-retry", id)
...
func TestDoWithRetry_NoRetryOn4xx(t *testing.T) {
	...
	c := newTestClientWithFastRetry(t, server.URL)
	_, err := c.CreateExperiment(context.Background(), "bad-exp", nil)
	require.Error(t, err)
