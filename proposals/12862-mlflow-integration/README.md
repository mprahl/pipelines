...
**Kubernetes-native mode.** In this mode, the MLflow server itself (not KFP) enforces authorization via the Kubernetes
[workspace provider](https://github.com/opendatahub-io/mlflow/tree/master/kubernetes-workspace-provider), which
performs `SelfSubjectAccessReview` checks against the `mlflow.kubeflow.org` API group for each incoming API request.
KFP does not perform its own authorization gate before calling MLflow. The minimum permissions required for the KFP
integration are `get`, `list`, `create`, and `update` on `experiments` in `mlflow.kubeflow.org`. Run operations (create,
update, log) are authorized under `experiments` permissions.
...
- **`"kubernetes"`** (default): KFP components use their Kubernetes service account tokens for MLflow API calls: the
  API server uses its own service account, while drivers/launchers and user containers use the pipeline run's service
  account in the run namespace. Workspaces default to enabled, mapping Kubernetes namespaces to MLflow workspaces. This
  assumes MLflow
